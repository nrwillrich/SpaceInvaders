...
{
    public class Player2 : Dynamic
    {
        bool m_isAlive = true;

        const float m_playerSpeed = 200.0f;
        //public float m_playerDying = 2.0f;

        //double m_animationTime = 0.0f;

        public Player2(World world, Vector2 pos, Vector2 size, Texture2D tex)
        ...
        public override bool Update(GameTime gameTime)
        {
            if (m_isAlive)
            {
                ...
                //Tiro do Player------------------------------------------------------------------------------------------------------
                if (GamePad.GetState(PlayerIndex.One).IsButtonDown(Buttons.A) && !m_world.m_prevGamePadState.IsButtonDown(Buttons.A))
                {
        ...
        public override void Draw(GameTime gameTime, SpriteBatch spriteBatch)
        {
            ...
            if (!m_isAlive)
            {
                m_world.m_spriteBatch.Draw(m_world.m_texPlayer2, m_pos, d, Color.White, 0.0f, new Vector2(d.Width, d.Height) * 0.5f,
                    Vector2.One, SpriteEffects.None, 0.0f);
            }

            else
                m_world.m_spriteBatch.Draw(m_world.m_texPlayer2, m_pos, p, Color.White, 0.0f, new Vector2(p.Width, p.Height) * 0.5f,
                    Vector2.One, SpriteEffects.None, 0.0f);
